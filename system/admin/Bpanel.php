...
if ($uid) {//Trae información del usuario, carga lista de vistas a disposición.
    $usuario_stmt = $pdo->prepare("SELECT id, estado, nombre, apellido, username, fecha_nacimiento, TIMESTAMPDIFF(YEAR, fecha_nacimiento, CURDATE()) AS edad FROM usuarios WHERE id = ? ORDER BY id");
